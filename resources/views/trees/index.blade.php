...
@section('content')
    ...
    <div class="container-fluid">
        ...
        @foreach($championships as $championship)
            ...
            @if ($championship->tree != null)
                @foreach($championship->tree->groupBy('area') as $ptByArea)
                    <?php
                    $ptByAreaByc5 = $ptByArea->filter(function ($value, $key) {
                        return $value->c5 != null;
                    });
                    $ptByAreaByc4 = $ptByArea->filter(function ($value, $key) {
                        return $value->c4 != null;
                    });
                    $ptByAreaByc3 = $ptByArea->filter(function ($value, $key) {
                        return $value->c3 != null;
                    });
                ...
                @endforeach
            @else
                <div>No hay torneo generado</div>
            @endif
        @endforeach
    </div>
